...
static void *new_thread_start(void *arg) 
{
	...
	while(i++<20){
		sleep(1);
		len = sprintf(buffer,"helloworld!-->%d\r\n",i);
		write(fd, buffer, len);
	}
	return (void *)0; 
}
...
int main(int argc, char *argv[])
{
	...
	char *device = NULL;
	int rw_flag = -1;
	unsigned char w_buf[11] = {0x31, 0x32, 0x33, 0x34,
		0x35, 0x36, 0x37, 0x38, '\r','\n','\0'};    //通过串口发送出去的数据
	int n;
	pthread_t tid;
	void *tret;
	int ret = 0;
	...
	switch (rw_flag) {
		case 0:  //读串口数据
			...
			write(fd,"start new thread to print:\r\n",28);
...
//			}
	...
	printf("waiting thread ID:%d  quit...\n",tid);
